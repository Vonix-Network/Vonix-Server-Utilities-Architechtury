...
public final class LinkCommands {
    ...
    private static int linkExecute(CommandContext<CommandSourceStack> ctx) {
        ...
        if (client == null || !client.getConfig().isEnabled()) {
            player.sendMessage(new TextComponent("§c[VSU] Site link feature disabled by server admin."), net.minecraft.Util.NIL_UUID);
            return 0;
        }
        ...
        if (prev != null && (now - prev) < cooldownMs) {
            ...
            player.sendMessage(new TextComponent(
                    "§c[VSU] Please wait §e" + remain + "s §cbefore requesting another link code."), net.minecraft.Util.NIL_UUID);
            return 0;
        }
        lastRequest.put(uuid, now);

        String username = player.getName().getString();
        player.sendMessage(new TextComponent("§7[VSU] Requesting link code…"), net.minecraft.Util.NIL_UUID);
        ...
        client.generateLinkCode(uuid, username).thenAccept(json -> {
            ...
            player.server.execute(() -> player.sendMessage(
                    new TextComponent("§c[VSU] Site unreachable, try again in a minute."), net.minecraft.Util.NIL_UUID));
            return null;
        });
        return 1;
    }
    ...
    private static void handleLinkResponse(ServerPlayer player, JsonObject json) {
        if (json == null
                ...
                || !json.has("code")) {
            player.sendMessage(new TextComponent("§c[VSU] Site unreachable, try again in a minute."), net.minecraft.Util.NIL_UUID);
            return;
        }
        ...
        MutableComponent codeChip = new TextComponent(code).setStyle(Style.EMPTY
        ...
        MutableComponent urlChip = new TextComponent(LINK_URL).setStyle(Style.EMPTY
                .withColor(ChatFormatting.AQUA)
                .withUnderlined(true)
                .withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, LINK_URL))
                .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                        new TextComponent("Open in browser"))));

        player.sendMessage(new TextComponent("§6§l[VSU] §r§eYour Vonix link code:"), net.minecraft.Util.NIL_UUID);
        player.sendMessage(new TextComponent("    ").append(codeChip)
                .append(new TextComponent("  §7(click to copy)")), net.minecraft.Util.NIL_UUID);
        player.sendMessage(new TextComponent("§eGo to: ").append(urlChip), net.minecraft.Util.NIL_UUID);
        player.sendMessage(new TextComponent(
                "§7Paste the code on the site to finish linking. Expires in §e" + minutes + " min§7."), net.minecraft.Util.NIL_UUID);
    }
    ...
    private static int unlinkExecute(CommandContext<CommandSourceStack> ctx) {
        ...
        MutableComponent urlChip = new TextComponent("vonix.network/account/link-minecraft")
        ...
        ).append(urlChip).append(new TextComponent(" §eand click Unlink.")), net.minecraft.Util.NIL_UUID);
        return 1;
    }
    ...
    public static void appendStatusLines(CommandSourceStack source) {
        VenaryClient client = VenaryClient.get();
        if (client == null) {
            source.sendSuccess(new TextComponent("§7Venary: §cnot initialized"), false);
            return;
        }
        VenaryConfig cfg = client.getConfig();
        String state = cfg.isEnabled() ? "§aenabled" : "§cdisabled";
        source.sendSuccess(new TextComponent("§7Venary: " + state
                + "§7, base=§f" + cfg.getApiBase()
                + "§7, api_key=§f" + cfg.getMaskedApiKey()), false);
        source.sendSuccess(new TextComponent("§7  login-jwt=§f" + cfg.isLoginJwtEnabled()
