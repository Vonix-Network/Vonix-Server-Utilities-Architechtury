...
public final class UtilityCommands {
    ...
    private static int teleportTo(CommandContext<CommandSourceStack> ctx, ServerPlayer target) {
        ...
        player.teleportTo(target.level(), target.getX(), target.getY(), target.getZ(), java.util.Set.of(),
    ...
    private static int teleportPlayerTo(CommandContext<CommandSourceStack> ctx,
        ...
        target.teleportTo(dest.level(), dest.getX(), dest.getY(), dest.getZ(), java.util.Set.of(),
                dest.getYRot(), dest.getXRot(), false);
    ...
    private static int teleportHere(CommandContext<CommandSourceStack> ctx, ServerPlayer target) {
        if (!(ctx.getSource().getEntity() instanceof ServerPlayer player)) return 0;
        TeleportManager.getInstance().saveLastLocation(target);
        target.teleportTo(player.level(), player.getX(), player.getY(), player.getZ(), java.util.Set.of(),
    ...
    private static int teleportAll(CommandContext<CommandSourceStack> ctx) {
        if (!(ctx.getSource().getEntity() instanceof ServerPlayer player)) return 0;
        int count = 0;
        for (ServerPlayer t : player.level().getServer().getPlayerList().getPlayers()) {
            if (t != player) {
                TeleportManager.getInstance().saveLastLocation(t);
                t.teleportTo(player.level(), player.getX(), player.getY(), player.getZ(), java.util.Set.of(),
                        player.getYRot(), player.getXRot(), false);
    ...
    private static int setSpawn(CommandContext<CommandSourceStack> ctx) {
        ...
        player.level().setRespawnData(net.minecraft.world.level.storage.LevelData.RespawnData.of(player.level().dimension(), pos, 0.0F, 0.0F));
        player.sendSystemMessage(Component.literal(
    ...
    private static int replyMessage(CommandContext<CommandSourceStack> ctx, String message) {
        ...
        ServerPlayer target = sender.level().getServer().getPlayerList().getPlayer(lastUuid);
        if (target == null) {
    ...
    private static int openAccessory(CommandContext<CommandSourceStack> ctx, ServerPlayer target) {
        if (!(ctx.getSource().getEntity() instanceof ServerPlayer player)) return 0;
        return AccessoryHelper.openAccessoryMenu(target, player) ? 1 : 0;
    }
